...
//! and this module is everything behind that question: what the latest release
...
//! to a running daemon on the new build without losing the session.
//!
...
//!
//! [`apply`] stops the daemon *before* it replaces the binary, and that is not
//! arbitrary. [`crate::conn::spawn_daemon`] finds the daemon with
//! `std::env::current_exe()`, and on Linux, once a running executable has been
//! renamed over, `/proc/self/exe` reads `".../butai (deleted)"` — a path that
//! does not exist. Anything that spawned a daemon between the swap and the
//! exec would fail on it. So: kill, wait for it to actually be gone, swap,
//! exec an explicitly resolved path.
//!
...
impl Drop for Staged {
    ...
    /// `~/.local/bin`. Dropping it cleans up; [`apply`] renames it away first,
